...
@implementation MudVariablesDetail


@synthesize tableview, vartitle, varcurrentvalue;//, varinitialvalue;



-(void)SaveButtonClicked {
	MudClientIpad4AppDelegate  *myAppDelegate = (MudClientIpad4AppDelegate *) [[UIApplication sharedApplication] delegate];
	MudVariables *variableView = [self.navigationController.viewControllers objectAtIndex:[self.navigationController.viewControllers count]-2];
	...
	if (!variableView.element) {
		...
		MudDataVariables *char1 = (MudDataVariables *)[NSEntityDescription insertNewObjectForEntityForName: @"MudDataVariables" inManagedObjectContext:myAppDelegate.context];
		...
		int topIndex = 0;
		for (MudDataVariables *v in session.variables) {
	...
	if ([self.vartitle.text length]) {
		[variableView.element setTitle:self.vartitle.text];
	} else {
		[variableView.element setTitle:@"!invalid!"];
	}
	
	
	if ([self.varcurrentvalue.text length]) {
		[variableView.element setCurrentValue:self.varcurrentvalue.text];
	} else {
		[variableView.element setCurrentValue:@""];
	}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (section == 0) {
		...
		if (cell == nil) {
			...
			if (variableView.element) {
				[textField setText:[variableView.element title]];
			}
		...
		if (cell == nil) {
			...
			CGRect textFieldFrame = CGRectMake(0.0, 0.0, cell.bounds.size.width, 240.0);
			...
			if (variableView.element) {
				[textView setText:[variableView.element currentValue]];
			}
...
- (void)loadView {
	...
	if (variableView.element) {
		self.navigationItem.title = @"Edit Variable";
		...
		self.navigationItem.title = @"Add Variable";
		UIBarButtonItem *rb = [[UIBarButtonItem alloc] initWithTitle:@"Save" style:UIBarButtonItemStyleDone target:self action:@selector(SaveButtonClicked)];
		self.navigationItem.rightBarButtonItem = rb;
	}
